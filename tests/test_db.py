...
class TestDatabaseRelationships:
    ...
    def test_user_task_relationship(self, app_context, db):
        ...
        db.execute(
            "INSERT INTO user (username, email, password, name, role) VALUES (?, ?, ?, ?, ?)",
            ('reluser', 'rel@test.com', 'pass', 'Rel User', 'volunteer')
        )
        # Create task
        db.execute(
            "INSERT INTO task (task_name, description, status) VALUES (?, ?, ?)",
            ('Rel Task', 'Description', 'pending')
        )
        db.commit()

        user = db.execute("SELECT user_id FROM user WHERE username = 'reluser'").fetchone()
        task = db.execute("SELECT task_id FROM task WHERE task_name = 'Rel Task'").fetchone()
...
class TestDatabaseSeedData:
    """Tests for seeded test data from schema."""

    def test_seed_tasks_created(self, app_context, db):
        """Test that seed tasks are created."""
        tasks = db.execute("SELECT * FROM task").fetchall()
        # Schema inserts 3 tasks
        assert len(tasks) >= 3

    def test_seed_task_statuses(self, app_context, db):
        """Test that seed tasks have various statuses."""
        tasks = db.execute("SELECT status FROM task").fetchall()
        statuses = [t['status'] for t in tasks]
        # Should have pending, in_progress, and finished
        assert 'pending' in statuses or 'in_progress' in statuses or 'finished' in statuses

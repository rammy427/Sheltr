...
| `create()` | name, description, status, volunteer_id, shelter_id | (Task, error) | Create new task |
...
| `update_status()` | status | (bool, error) | Change status |
...
| `validate_completion_date()` | Valid timestamp format |
